...
TEST(VEC3_BASIC, Test_Operator_equal) {
  ...
  ASSERT_EQ(Vec3(std::sqrt(2.0f), 0, 1.001),
            Vec3(1.4142135623730950488016887242097, 0, 1.001));
}
...
TEST(VEC3_OPERATOR_PLUS, Test_Operator_Rvalue_Addition) {
  ...
  ASSERT_EQ(Vec3(0.1, 0.2, 0.3),
            Vec3(0, 0.2, 0.3) + std::move(Vec3(0.1, 0, 0)));
  ASSERT_EQ(Vec3(0, 1, 0), std::move(Vec3(0, 0, 0) + std::move(Vec3(0, 1, 0))));
  ASSERT_EQ(Vec3(1, 0, -0.1),
            std::move(Vec3(1, 0, 1)) + std::move(Vec3(0, 0, -1.1)));
}
...
TEST(VEC3_OPERATOR_PLUS, Test_Operator_plus_Equal_rvalue_Vec3) {
  ASSERT_EQ(Vec3(0.2, 0.2, 0.3),
            Vec3(0.1, 0, 0.2) += std::move(Vec3(0.1, 0.2, 0.1)));
  ASSERT_EQ(Vec3(0.1, 0.5, -0.3),
            Vec3(-0.1, 0.3, -5.3) += std::move(Vec3(0.2, 0.2, 5)));
}
...
TEST(VEC3_OPERATOR_MINUS, Test_Operator_rvalue_Minus_Vec3_Vec3) {
  ASSERT_EQ(Vec3(0, 0, -1),
            std::move(Vec3(0, 0, 0)) - std::move(Vec3(0, 0, 1)));
  ASSERT_EQ(Vec3(0, 0.1, 0),
            std::move(Vec3(0, 0.1, 0.1) - std::move(Vec3(0, 0, 0.1))));
  ASSERT_EQ(Vec3(0, 1, 0.03),
            std::move(Vec3(0, 5, 0.05) - std::move(Vec3(0, 4, 0.02))));
}
...
TEST(VEC3_OPERATOR_MINUS, Test_Operator_Minus_Rvalue_Vec3_Rvalue_float) {
  ASSERT_EQ(Vec3(0.1, 0.2, 0.1),
            std::move(Vec3(0.2, 0.3, 0.1)) - std::move(Vec3(0.1, 0.1, 0)));
  ASSERT_EQ(Vec3(0.2, 0.3, 0.4),
            std::move(Vec3(0.1, 0.2, 0.3)) - std::move(Vec3(-0.1, -0.1, -0.1)));
  ASSERT_EQ(Vec3(0.1, 0.1, 0.1),
            std::move(Vec3(0.8, 0.1, 0.3)) - std::move(Vec3(0.7, 0, 0.2)));
}

TEST(VEC3_OPERATOR_MINUS, Test_Operator_Minus_Rvalue_Vec3_Rvalue_float2) {
  ASSERT_EQ(Vec3(0.1, 0.2, 0.3),
            std::move(Vec3(0.2, 0.3, 0.4)) - std::move(0.1));
  ASSERT_EQ(Vec3(0.2, 0.3, 0.1),
            std::move(Vec3(0.1, 0.2, 0)) - std::move(-0.1));
  ASSERT_EQ(Vec3(0.3, 0.1, 0.1),
            std::move(Vec3(0.5, 0.3, 0.3)) - std::move(0.2));
}
...
TEST(VEC3_OPERATOR_MINUS, Test_Operator_minus_equal_Rvalue_Vec3) {
  ASSERT_EQ(Vec3(0.4, 0.1, 0.2),
            Vec3(0.5, 0.8, 0.3) -= std::move(Vec3(0.1, 0.7, 0.1)));
  ASSERT_EQ(Vec3(0.5, 0.5, 0.8),
            Vec3(0.8, 0.7, 80) -= std::move(Vec3(0.3, 0.2, 79.2)));
  ASSERT_EQ(Vec3(0.4, 1, -40.1),
            Vec3(0.9, 0.3, 40) -= std::move(Vec3(0.5, -0.7, 80.1)));
}
...
TEST(VEC3_OPERATOR_MULTIPLY, Test_Operator_multiply_rvalue_float_rvalue_vec3) {
  ...
  ASSERT_EQ(Vec3(0.3, 0.2, 0.1),
            std::move(0.001) * std::move(Vec3(300, 200, 100)));
}
...
TEST(VEC3_OPERATOR_MULTIPLY, Test_Operator_multiply_rvalue_vec3_rvalue_float) {
  ...
  ASSERT_EQ(Vec3(0.3, 0.2, 0.1),
            std::move(Vec3(300, 200, 100)) * std::move(0.001));
}
...
TEST(VEC3_OPERATOR_DIVISION,
     Test_Operator_Division_Throw_exception_division_by_zero) {
...
TEST(VEC3_OPERATOR_DIVISION,
     Test_Operator_Division_Throw_exception_rvalue_division_by_zero_) {
  ASSERT_THROW(std::move(Vec3(0, 0, 1)) / 0, std::logic_error);
  ASSERT_THROW(std::move(Vec3(0, 0, 0)) / (constants::kAccuracy * 0.1),
               std::logic_error);
  ASSERT_THROW(std::move(Vec3(0, 0, 0)) / (constants::kAccuracy * 0.5),
               std::logic_error);
  ASSERT_THROW(std::move(Vec3(0, 0, 0)) / (constants::kAccuracy * 0.9),
               std::logic_error);
  ASSERT_THROW(std::move(Vec3(0, 0, 0)) / constants::kAccuracy,
               std::logic_error);
}

TEST(VEC3_OPERATOR_DIVISION,
     Test_Operator_Division_Throw_exception_division_by_rvalue_zero) {
  ASSERT_THROW(Vec3(0, 0, 1) / std::move(0), std::logic_error);
  ASSERT_THROW(Vec3(0, 0, 0) / std::move(constants::kAccuracy * 0.1),
               std::logic_error);
  ASSERT_THROW(Vec3(0, 0, 0) / std::move(constants::kAccuracy * 0.5),
               std::logic_error);
  ASSERT_THROW(Vec3(0, 0, 0) / std::move(constants::kAccuracy * 0.9),
               std::logic_error);
  ASSERT_THROW(Vec3(0, 0, 0) / std::move(constants::kAccuracy),
               std::logic_error);
}

TEST(VEC3_OPERATOR_DIVISION,
     Test_Operator_Division_Throw_exception_rvalue_division_by_rvalue_zero) {
  ASSERT_THROW(std::move(Vec3(0, 0, 1)) / std::move(0), std::logic_error);
  ASSERT_THROW(std::move(Vec3(0, 0, 0)) / std::move(constants::kAccuracy * 0.1),
               std::logic_error);
  ASSERT_THROW(std::move(Vec3(0, 0, 0)) / std::move(constants::kAccuracy * 0.5),
               std::logic_error);
  ASSERT_THROW(std::move(Vec3(0, 0, 0)) / std::move(constants::kAccuracy * 0.9),
               std::logic_error);
  ASSERT_THROW(std::move(Vec3(0, 0, 0)) / std::move(constants::kAccuracy),
               std::logic_error);
}
...
TEST(VEC3_METHODS, Test_method_crossProduct) {
  ASSERT_EQ(Vec3(5, 0.3, 0.1).crossProduct(Vec3(6, 3, 2)),
            Vec3(0.3, -9.4, 13.2));
  ASSERT_EQ(Vec3(4, 8, 0.1).crossProduct(Vec3(26, -5, 2)),
            Vec3(16.5, -5.4, -228));
  ASSERT_EQ(Vec3(3, 0.18, 50).crossProduct(Vec3(13, -8, 3)),
            Vec3(400.54, 641, -26.34));
}
...
TEST(VEC3_METHODS, Test_method_normalize) {
  ...
  ASSERT_EQ(Vec3(4, 0, 2).normalize(),
            Vec3(2 / std::sqrt(5.f), 0, 1 / sqrt(5)));
}
